      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
              ...
              <div>
                <div className="flex items-center space-x-1.5">
                  <span className="font-bold text-slate-900 text-lg tracking-tight">
                    ProjectMatch<span className="text-indigo-600 font-extrabold">.AI</span>
                  </span>
              ...
              >
                ...
                {activeProject && activeProject.selectedTeamMemberIds.length > 0 && (
                  <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                    currentTab === "matching" ? "bg-white text-indigo-700" : "bg-indigo-100 text-indigo-800"
                  }`}>
          ...
          <div className="flex items-center space-x-3">
            ...
            {projects.length > 0 && (
              <div className="hidden lg:flex items-center text-xs bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5">
                ...
                <select
                  ...
                  className="bg-transparent font-semibold text-slate-800 focus:outline-none cursor-pointer max-w-[170px] truncate pr-1"
                >
                  {projects.map((p) => (
            ...
            <button
              ...
              onClick={() => setCurrentTab("profile")}
              className="flex items-center space-x-2 pl-2 pr-1 py-1 rounded-full hover:bg-slate-100 transition-colors cursor-pointer border border-slate-200"
        ...
        <div className="flex md:hidden items-center justify-around py-2 border-t border-slate-100 overflow-x-auto space-x-1">
          <button
            onClick={() => setCurrentTab("landing")}
            className={`px-2.5 py-1.5 rounded-md text-xs font-medium ${
              currentTab === "landing" ? "bg-slate-100 text-indigo-700 font-bold" : "text-slate-600"
            }`}
          >
            Home
          </button>
          <button
            onClick={() => setCurrentTab("dashboard")}
            className={`px-2.5 py-1.5 rounded-md text-xs font-medium ${
              currentTab === "dashboard" ? "bg-slate-100 text-indigo-700 font-bold" : "text-slate-600"
            }`}
          >
            Dashboard
          </button>
          <button
            onClick={() => setCurrentTab("projects")}
            className={`px-2.5 py-1.5 rounded-md text-xs font-medium ${
              currentTab === "projects" ? "bg-slate-100 text-indigo-700 font-bold" : "text-slate-600"
            }`}
          >
            Projects
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-semibold ${
              currentTab === "matching" ? "bg-indigo-600 text-white" : "text-indigo-600 font-bold"
            }`}
          >
            ✨ Find Team
          </button>
          <button
            onClick={() => setCurrentTab("profile")}
            className={`px-2.5 py-1.5 rounded-md text-xs font-medium ${
              currentTab === "profile" ? "bg-slate-100 text-indigo-700 font-bold" : "text-slate-600"
            }`}
          >
            Profile
          </button>
